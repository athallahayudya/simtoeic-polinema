@empty($alumni)
    ...
    @else
    <form action="{{ url('/manage-users/alumni/' . $alumni->alumni_id . '/delete_ajax') }}" method="POST" id="form-delete" class="d-inline">
        @csrf
        @method('POST')
        <div id="modal-master" class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Delete Alumni Data</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    ...
                    <table class="table table-sm table-bordered table-striped">
                        ...
                        <tr>
                            ...
                            <td class="col-9">{{ $alumni->nik }}</td>
                        </tr>
                        <tr>
                            <th class="text-right col-3">Exam Status :</th>
                            <td class="col-9">{{ $alumni->user->exam_status }}</td>
                        </tr>
                        <tr>
                            <th class="text-right col-3">KTP Scan :</th>
                            <td class="col-9">{{ $alumni->ktp_scan }}</td>
                        </tr>
                        <tr>
                            <th class="text-right col-3">Photo :</th>
                            <td class="col-9">{{ $alumni->photo }}</td>
                        </tr>
                        <tr>
                            <th class="text-right col-3">Home Address :</th>
                            <td class="col-9">{{ $alumni->home_address }}</td>
                        </tr>
                        <tr>
                            <th class="text-right col-3">Current Address :</th>
                            <td class="col-9">{{ $alumni->current_address }}</td>
                        </tr>
                    </table>
                </div>
                    ...
                    success: function(response) {
                        if (response.status === true) {
                            ...
                            Swal.fire({
                                ...
                                if (typeof dataAlumni !== 'undefined' && dataAlumni.ajax.reload) {
